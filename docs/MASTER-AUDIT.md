...
- index.html = **2,496,321 bytes (~2.4 MB)**, 2,865 lines, minified CSS+JS inline, as measured at this audit's own commit (`6b5f811`, 2026-08-23). Confirmed monolith. This is a point-in-time figure, not a live one — the site was rebuilt substantially afterward (fabricated widgets removed, research engine merged in, ongoing maintenance since) and the `pj` branch's current index.html is **210,207 bytes (~205 KB)**, 3,330 lines. Do not read the 2.4 MB figure as describing the file today.
