// The three legal pages (privacy, terms, risk-disclosure) share one
// byte-identical nav and light-mode CSS block, all ship assets/theme.js's
// default-light bootstrap, but never got a #theme-toggle control -- the
// same gap already fixed once for stock-lab/research-engine/
// premium-guidance (tests/theme-toggle-tools.test.mjs). None of these three
// load curriculum.js, so each got the same small self-contained wiring
// script copied verbatim.
...
const PAGES = ['privacy.html', 'terms.html', 'risk-disclosure.html'];

test('each legal page ships a #theme-toggle button', () => {
...
// "slice between two literal markers" approach the tool-page test uses.
...
for (const page of PAGES) {
  ...
  test(`${page}: the toggle button's colors are overridden in light mode too`, () => {
    const html = read(page);
    assert.match(
      html,
      /body\.light-mode #theme-toggle\{background:var\(--bg2\);color:var\(--muted\);border-color:var\(--border\);\}/,
      `${page} is missing the light-mode #theme-toggle override`,
    );
  });
}
